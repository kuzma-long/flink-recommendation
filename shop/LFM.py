...
def latenFactorModel(df, classCount, iterCount, alpha, lamda):
    '''
    隐语义模型计算参数p,q
    :param df: 源数据
    :param classCount: 隐类数量
    ...
    :return: 参数p,q
    '''
    p, q, userItem = initModel(df, classCount)
    for step in range(0, iterCount):
        for user in userItem:
            for uid, samples in user.items():
                for iid, rating in samples.items():
                    delta = rating - lfmPredict(p, q, uid, iid)
                    for f in range(0, classCount):
                        print('step %d user %d class %d' % (step, uid, f))
                        p[f][uid] += alpha * (delta * q[iid][f] - lamda * p[f][uid])
                        q[iid][f] += alpha * (delta * p[f][uid] - lamda * q[iid][f])
        alpha *= 0.9
    return p, q
